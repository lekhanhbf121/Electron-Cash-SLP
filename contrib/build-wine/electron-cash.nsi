...
Section
  ...
  ;Links simpleledger: URI's to Electron Cash SLP
  WriteRegStr ${INSTDIR_REG_ROOT} "Software\Classes\simpleledger" "" "URL:simpleledger Protocol"
  WriteRegStr ${INSTDIR_REG_ROOT} "Software\Classes\simpleledger" "URL Protocol" ""
  WriteRegStr ${INSTDIR_REG_ROOT} "Software\Classes\simpleledger" "DefaultIcon" "$\"$INSTDIR\electron.ico, 0$\""
  WriteRegStr ${INSTDIR_REG_ROOT} "Software\Classes\simpleledger\shell\open\command" "" "$\"$INSTDIR\${INTERNAL_NAME}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey ${INSTDIR_REG_ROOT} "Software\Classes\simpleledger"
